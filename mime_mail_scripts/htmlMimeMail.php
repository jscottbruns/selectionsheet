...
class Client
{
	...
	var $message_body;
	var $msg_num;
	function open_mail($pop3_address, $username, $password, $msg_num)
	{
		$this->msg_num = $msg_num;
		$this->mbox = imap_open("{" . $pop3_address . "/pop3}INBOX", $username, $password);
	...
	function get_body($msg_num)
	{
		...
		if(!eregi("quoted-printable", $body))
		{
			...
			$body = str_replace("3D", "", $body);
			$bodyarr = explode("quoted-printable", $body);
			$bodyarr = explode("------=", $bodyarr[1]);
			$this->message_body = $bodyarr[0];
		}
		$this->message_body = str_replace(array("\r", "\n"), array("<br>", ""), $this->message_body);
		while(substr($this->message_body, 0, 4) == "<br>")
			$this->message_body = substr($this->message_body, 4);
...
//					if(strlen($part->parameters[0]->value) > 0)
//						$this->attach_array[] = $part->parameters[0]->value;
	...
	function get_header($msg_num)
	{
		...
		if(eregi("<", $this->fromaddress))
		{
			...
			$this->fromaddress = "<a href='mailto:" . $head->fromaddress . "'>" . $head->fromaddress . "</a>";
		}
	}	
} // End of Client class
