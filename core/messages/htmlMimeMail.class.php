...
/**
*
* Last Modified April 07, 2005 - Bob Simpson
*
*/
...
/**
*
* Below added by Bob Simpson March 28, 2005
* 
*/
	function addAttachParts($attachstr, $attachfolder)
	{
		//$attacharr = explode("|", substr($attachstr, 0, -1));
		$attacharr = $attachstr;
		foreach ($attacharr as $filename)
		{
			$tmp_filename = $attachfolder . $filename;
			$content_type = $this->getContentType($filename);
			$attachment = $this->getFile($tmp_filename);
			if(strrev(substr(strrev($filename),0,4)) == '.txt')
				$this->addAttachment($attachment, $filename, $content_type, '7bit');
			else
				$this->addAttachment($attachment, $filename, $content_type);
		}
		unlink($tmp_filename);
	}
...
/**
* Get email class by Bob Simpson
*/
...
class Client
{
	...
	var $message_body;
	var $msg_num;
	function open_mail($pop3_address, $username, $password, $msg_num, $foldername=NULL)
	{
		if (!$foldername)
			$foldername = 'INBOX';
		$this->msg_num = $msg_num;
		if (!$foldername || $foldername == "INBOX") $foldername = "/pop3}INBOX";
		else $foldername = ":143/imap/notls}$foldername";
		$this->mbox = imap_open("{" . $pop3_address . $foldername, $username, $password);
		$this->msg_cnt = imap_num_msg($this->mbox);
		$this->get_header($this->msg_num);
		$this->get_parts($this->msg_num);
		$this->get_body($this->msg_num);
	}
	function get_body($msg_num)
	{
		$body = imap_fetchbody($this->mbox, $msg_num, 1);
		$pattern = "'(http[s]{0,1}://[^\s]+)'i"; 
		$replace = "<a href=\"\\1\" ".(!ereg("selectionsheet.com",$body) ? "target=\"_blank\"" : NULL).">\\1</a>"; 
		$body = preg_replace($pattern, $replace, $body); 		
		
		if(!eregi("quoted-printable", $body))
		{
			if(eregi("<body", $body))
			{
				$body = stripslashes($body);
				$body = str_replace("<BODY", "body", $body);
				$bodyarr = explode("<body", $body);
				$bodyarr[1] = substr($bodyarr[1], strpos($bodyarr[1], ">") +1);
				$body = $bodyarr[1];
			} else {
			...
			$body = str_replace(array("=3D20", "3D"), array("", ""), $body);
			
		} else {
			$body = quoted_printable_decode($body);
			$body = substr($body, strpos($body, 'quoted-printable') + 16);
			$body = substr($body, 0, strpos($body, '--'));
			$body = trim($body);
			$body = str_replace(array("=3D20", "\r", "3D"), array("", "<br />", ""), $body);
		}
		
		$this->message_body = $body;
	}
	function get_parts($msg_num)
	{
		$this->structure = imap_fetchstructure($this->mbox, $msg_num);
		$this->parts_count = count($this->structure->parts);
		if ($this->parts_count > 1) {
			foreach ($this->structure->parts as $part) {
				if ($part->disposition == "INLINE")
				{
					if($part->description)
						$this->attach_array[] = $part->description;
				} elseif ($part->disposition == "ATTACHMENT") {
					if(strlen($part->dparameters[0]->value) > 0)
						$this->attach_array[] = $part->dparameters[0]->value;
				} elseif ($part->disposition == "FILENAME") {
					$this->attach_array[] = $part->parameters[0]->value;
				}
			}
		}
		return $this->attach_array;
	}
	function get_header($msg_num)
	{
		$head = imap_headerinfo($this->mbox, $msg_num);
		$this->date = $head->date;
		$this->toaddress = $head->toaddress;
		$this->subject = $head->subject;
			
		if(eregi("<", $this->fromaddress))
		{
			$fromarr = explode("<", $this->fromaddress);
			$fromarr[1] = str_replace(">", "", $fromarr[1]);
			$this->fromaddress = $fromarr[0] . " <a href='mailto:" . $fromarr[1] . "'>" . $fromarr[1] . "</a>"; 
		} else {
			$this->fromemail = explode("<",$head->fromaddress);
			$this->fromemail[1]= str_replace(">","",$this->fromemail[1]);
			$this->fromaddress = $head->fromaddress;
		}
	}	
} // End of Client class
