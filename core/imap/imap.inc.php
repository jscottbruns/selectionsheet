<?php
	...
	class IMAPMAIL
	{
		...
		var $tag;
		var $mail_box;
		var $filesizename = array(" Bytes", " KB", " MB", " GB", " TB", " PB", " EB", " ZB", " YB");
		...
		function message_append($msg,$mailbox) {
			if($this->put_line($this->tag." APPEND $mailbox (\Seen) {".strlen($msg)."}"))
			{
				$this->put_line($msg);
			}
			else
		...
		function get_quotaroot() {
			...
			curl_setopt($ch, CURLOPT_POSTFIELDS, "op=4&u=".EMAIL_USERNAME); 
		...
		function get_header_field($msgno,$field) {
			$flag = "BODY.PEEK[HEADER.FIELDS ($field)]";
			$response=$this->fetch_mail($msgno,$flag);
			$id = explode($field.":",$response);
			$answer = substr(trim($id[1]),0,strpos($id[1],$this->tag)-4);
		...
		function get_envelope($msgno) {
			...
			//From:
			$match = preg_split("/[\"]+/", str_replace("))","",$matches[1])); 			
			$envelope['from']['name'] = $match[1];
			$envelope['from']['source_route'] = $match[2];
			$envelope['from']['mailbox'] = $match[3];
			$envelope['from']['domain'] = $match[5];
			//Sender:
			$match = preg_split("/[\"]+/", str_replace("))","",$matches[2])); 			
			$envelope['sender']['name'] = $match[1];
			$envelope['sender']['source_route'] = $match[2];
			$envelope['sender']['mailbox'] = $match[3];
			$envelope['sender']['domain'] = $match[5];
			//CC:
			$match = preg_split("/[\"]+/", str_replace("))","",$matches[5])); 			
			if ($match[1] || ($match[3] && $match[5])) {
				$envelope['cc']['name'] = $match[1];
				$envelope['cc']['source_route'] = $match[2];
				$envelope['cc']['mailbox'] = $match[3];
				$envelope['cc']['domain'] = $match[5];
			}
			//BCC:
			$match = preg_split("/[\"]+/", str_replace("))","",$matches[6])); 			
			if ($match[1] || ($match[3] && $match[5])) {
				$envelope['bcc']['name'] = $match[1];
				$envelope['bcc']['source_route'] = $match[2];
				$envelope['bcc']['mailbox'] = $match[3];
				$envelope['bcc']['domain'] = $match[5];
			}
			return $envelope;
		}

		function get_size($msgno) {
			$flag = "RFC822.SIZE";
			$response=$this->fetch_mail($msgno,$flag);
			$size = substr($response,strpos($response,$flag)+strlen($flag)+1);
				
			return intval($size);
		}
		
		function get_flags($msgno) {
			$response=$this->fetch_mail($msgno,"FLAGS");
			if (eregi("seen",$response))
				$flag[] = "seen";
			if (eregi("answered",$response))
				$flag[] = "answered";
			if (eregi("flagged",$response))
				$flag[] = "flagged";
			if (eregi("deleted",$response))
				$flag[] = "deleted";
			if (eregi("draft",$response))
				$flag[] = "draft";
			if (eregi("recent",$response))
				$flag[] = "recent";
      ...
      the connection. */
	   ...
	   mailbox can be accessed. */ 
		...
		function format_error_msg() {
			return substr($this->error,strpos($this->error,$this->tag)+strlen($this->tag)+4);	
		}
	  ...
	  */
		function list_mailbox($ref_mail_box="",$wild_card="*")
		{
			...
			{
				$line=trim($temp_arr[$i]);
				array_push($return_arr,trim(substr($line,strrpos($line," ")+1)));
			}
			return $return_arr;
		}
