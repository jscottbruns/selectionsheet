	    ...
	    */
		...
		function walk($object,$msg='',$main_content_type="")
		{
			...
			if(!isset($object->parts))
			{
				...
				switch($ctype) {
					case "text/html":
						$msg['text/html'].=$object->body;
						break;
					case "text/plain":
						$enc=$object->headers['content-transfer-encoding'];
						if($enc!="quoted-printable")
							$msg['text/plain'].=nl2br($object->body);
						else
							$msg['text/plain'].=nl2br(quoted_printable_decode($object->body));
						break;
					case "image/jpeg":
					case "image/gif":
						...
						{
							$msg['text/plain'].="<hr>";
							$msg['text/plain'].="<a href='".ATTACHMENT_LINK.$name."' target='_blank'>$name</a>";
							$msg['text/plain'].="<hr>";
							$msg['text/plain'].="<img src='".ATTACHMENT_LINK.$name."'>";			
							$msg['text/html'].="<hr>";
							$msg['text/html'].="<a href='".ATTACHMENT_LINK.$name."' target='_blank'>$name</a>";
							$msg['text/html'].="<hr>";
							$msg['text/html'].="<img src='".ATTACHMENT_LINK.$name."'>";			
						}
					...
					default:
						...
						while (file_exists($temp.$file_name.$extention))
							$file_name .= rand(1,500);
							
						$tmpfile=$temp.$file_name.$extention;
						//$tmpfile=realpath($tmpfile);
						$fp=fopen($tmpfile,"w");
						fwrite($fp,$object->body);
						fclose($fp);
						$msg['attachments'][] = array("link"	=>	"<a href='".ATTACHMENT_LINK.strrev(substr(strrev($tmpfile),0,strpos(strrev($tmpfile),"/")))."' target='_blank' title='Open file in a new window.'>$name</a> (".round(filesize($tmpfile)/pow(1024, ($i = floor(log(filesize($tmpfile), 1024)))), 2) . $this->filesizename[$i].")",
													  "path"	=>	$tmpfile,
													  "name"	=>	$name
													  );
						break;
