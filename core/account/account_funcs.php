...
function getMyAccountInfo() {
	...
	$result = $db->query("SELECT * FROM `user_login` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
	return $db->fetch_assoc($result);;
}
function status_name($status) {
	global $db;
	
	$result = $db->query("SELECT `status` FROM `task_status_names` WHERE `code` = '$status'");
...
function getMobileCarriers() {
	...
	$result = $db->query("SELECT `carrier_hash` , `name` FROM `mobile_carriers` ORDER BY `name`");
...
function mobileExists($number) {
	...
	$result = $db->query("SELECT `confirmed` FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `number` = '$number'");
...
function addNewMobile() {
	...
	if ($btn == "SEND CONFIRMATION CODE") {
		if ($_POST['carrier']) {
			...
			$db->query("INSERT INTO `mobile_device` (`id_hash` , `number` , `carrier` , `confirmed`) VALUES ('".$_SESSION['id_hash']."' , '$number' , '$carrier' , '$code')");
			
			$result = $db->query("SELECT `url` FROM `mobile_carriers` WHERE `carrier_hash` = '$carrier'");
			$url = $db->result($result);
			
			$mobileEmail = $number."@".$url;
			sendConfirmCode($mobileEmail,$code);
		...
		if ($_POST['confirm_code']) {
			...
			$result = $db->query("SELECT `number` FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `confirmed` = '$code'");
			...
			if ($row['number'] != $number || !$row['number']) {
				...
				$db->query("UPDATE `mobile_device` SET `confirmed` = '1' WHERE `id_hash` = '".$_SESSION['id_hash']."' && `number` = '$number'");
		...
		$db->query("DELETE FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `number` = '$number'");
...
function updategeneralinfo() {
	...
	if ($_POST['first_name'] && $_POST['last_name'] && $_POST['company'] && $_POST['street1'] && $_POST['city'] && $_POST['state'] && $_POST['zip'] && $_POST['phone1a'] && $_POST['phone1b'] && $_POST['phone1c'] && $_POST['timezone']) {
		...
		if ($_POST['current_password']) {
			...
			$result = $db->query("SELECT COUNT(*) AS Total FROM `user_login` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `password` = '".md5($_POST['current_password'])."'");
			...
			$result = $db->query("SELECT COUNT(*) AS Total FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
			
			if ($db->result($result) > 0) {
				if (!$mobile1 && !$mobile2) {
					$db->query("DELETE FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
				} elseif ($mobile1 || $mobile2) {
					$result = $db->query("SELECT `number` FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
					while ($row = $db->fetch_assoc($result)) 
						$currentMobile[] = $row['number'];
										
					for ($i = 0; $i < count($currentMobile); $i++) {
						if ($currentMobile[$i] != $mobile1 && $currentMobile[$i] != $mobile2) 
							$db->query("DELETE FROM `mobile_device` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `number` = '".$currentMobile[$i]."'");
			...
			$_REQUEST['redirect'] = "?cmd=general&p=0&feedback=$feedback";
	} else {
...
function getStyles() {
	...
	$result = $db->query("SELECT `status` , `style` FROM `task_status` WHERE `id_hash` = 'admin'");
	while ($row = $db->fetch_assoc($result)) {
		$result2 = $db->query("SELECT `style` FROM `task_status` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `status` = '".$row['status']."'");
...
function getColor($code) {
	...
	$result = $db->query("SELECT COUNT(*) AS Total FROM `task_status` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `status` = '$code'");
...
function statusName($code) {
	...
	$result = $db->query("SELECT `status` FROM `task_status_names` WHERE `code` = '$code'");
...
function updateAccountSched() {
	...
	if ($_POST['accountSchedBtn'] == "RESTORE DEFAULTS") {
		//When we restore defaults, we remove all rows owned by the user
		$db->query("DELETE FROM `task_status` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
		
	} elseif ($_POST['accountSchedBtn'] == "UPDATE") {
		//Schedule reminders and appointments
		require_once('include/user_prefs.class.php');
		...
		if ($_POST['cmd'] == "home") {
			...
			while (list($key,$value) = each($styles)) {
				if ($_POST[$key."_changed"]) {
					...
					$result = $db->query("SELECT COUNT(*) AS Total FROM `task_status` WHERE `id_hash` = '".$_SESSION['id_hash']."' && `status` = '".$key."'");
					
					if ($db->result($result) > 0)
						$db->query("UPDATE `task_status` SET `style` = '$style' WHERE `id_hash` = '".$_SESSION['id_hash']."' && `status` = '".$key."'");
					 else 
						$db->query("INSERT INTO `task_status` (`id_hash` , `status` , `style`) VALUES ('".$_SESSION['id_hash']."' , '".$key."' , '$style')");
	...
	$_REQUEST['redirect'] = "?cmd=".$_POST['cmd']."&p=".$_POST['p']."&feedback=$feedback";
...
function midnightOn() {
	...
	$result = $db->query("SELECT `sched_midnight` AS Total FROM `user_login` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
...
function midnightNotify() {
	...
	$result = $db->query("SELECT `sched_midnight_notify` FROM `user_login` WHERE `id_hash` = '".$_SESSION['id_hash']."'");
