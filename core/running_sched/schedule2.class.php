...
class schedule extends library {
	...
	*/////////////////////////////////////////////////////////////////////////////////////
	...
	function set_current_lot($lot_hash,$community=NULL) {
		...
		$result = $db->query("SELECT user_profiles.id_hash as profile_owner , lots.profile_id , lots.profile_hash , `start_date` , `lot_no` ".(!$community ? ", `community`" : NULL)." , lots.task , lots.phase , lots.duration , `sched_status` , `comment` 
							  FROM `lots`
		...
		if ($db->result($result) == 1) {
			...
			if ($undo_task && $undo_phase && $undo_duration && $undo_sched_status && $undo_comments) {
				...
				$db->query("DELETE FROM `task_logs`
							WHERE `id_hash` = '".$this->current_hash."' && `lot_hash` = '$undoLot' && `timestamp` = '$undo_timestamp'");
			} 
		} 
	...
	*/////////////////////////////////////////////////////////////////////////////////////
	function running_schedule($community_hash,$community,$extra=NULL,$prefs=NULL) {
		...
		<table class=\"sched_main\" cellspacing=\"1\" cellpadding=\"0\" ".($this->p ? "border=\"1\"" : "style=\"background-color:#9c9c9c;\"")." >".(!$this->p || $this->p == 2 ? "
			<tr>
				<td colspan=\"".($loop + 1)."\">".$this->WriteHead($StartDate,$loop,$DaysToAd,$community_hash)."</td>
			</tr>" : NULL);
		...
		for ($a = 0; $a < count($community['lots']); $a++) {
			...
			if ($_REQUEST['action'] == "edit_task" && $this->edit_lot == $this->lot_hash) {
				$tbl .= "
				<tr>
					<td colspan=\"".($loop + 1)."\" style=\"background-color:#ffffff;\">
						".$_REQUEST['cmdTbl']."
					</td>
				</tr>";
			}
			...
			<tr>".($view != 3 && !$this->p ? "
				<td class=\"sched_rowHead\" style=\"padding:6px;background:#fff url(images/bkgrnd_grey.png) repeat;\">LOT</td>" : 
				($rowCounter > 1 ? "
				<tr>
					<td colspan=\"".($loop + 1)."\">".$this->WriteHead($StartDate,$loop,$DaysToAd,$community_hash)."</td>
				</tr>" : NULL).(!$this->p ? "
			...
			*/
			if (!$this->p && $view != 3) {
				...
				<tr>
					<td class=\"sched_lotHead\" style=\"background-color:".($_REQUEST['action'] == "edit_task" && $this->lot_hash == $this->edit_lot ? "yellow;" : "#ffffff;")."\">
	...
	function running_schedule_wrap($community_hash,$community,$extra=NULL,$prefs=NULL) {
		...
		<table class=\"sched_main\" cellspacing=\"1\" cellpadding=\"0\" ".($this->p ? "border=\"1\"" : "style=\"background-color:#9c9c9c;\"")." >".(!$this->p || $this->p == 2 ? "
			<tr>
				<td colspan=\"".($span_loop + 1)."\">".$this->WriteHead($StartDate,$loop,$DaysToAd,$community_hash)."</td>
			</tr>" : NULL);
	...
	function running_schedule_wrap_printable($community_hash,$community,$extra=NULL,$prefs=NULL) {
		...
		$tbl .= "
		<table bgcolor=\"#003366\" cellspacing=\"1\" cellpadding=\"0\" border=\"1\" width=\"100%\">
			<tr>
				<td valign=\"middle\" bgcolor=\"#003366\" align=\"center\">
					<font color=\"#ffffff\"><strong>".date("M Y",strtotime($StartDate))."</strong></font>
				</td>
			</tr>";
		$reminders = array(2,5,8);
		$appts = array(6);
		...
		for ($a = 0; $a < count($community['lots']); $a++) {
			...
			for ($i = 0; $i < $loop; $i++) {
				if ($i < 7)
					...
					<td align=\"center\" bgcolor=\"#ffffff\">
						".date("D d",strtotime("$StartDate $DaysToAd[$i]"))."
					</td>";
				else 
					$tbl_row2 .= "
					<td align=\"center\" bgcolor=\"#ffffff\">
			...
			for ($p = 1; $p < 2; $p++) {
				for ($i = 0; $i < $loop; $i++) {
					...
					<tr>$tbl_row2</tr>
					<tr>" : NULL)."
					<td valign=\"top\" align=\"left\" ".($dayNumber < 1 ? "height=\"75\"" : NULL)." bgcolor=\"".$this->GetDayColor($dayNumber,$this->current_lot['start_date'],$holiday_date)."\">
						<table cellpadding=\"5\" bgcolor=\"#d9d9d9\" width=\"100%\">
							<tr>
								<td nowrap>".
								($dayNumber > 0 ? "
									<strong>Day: $dayNumber</strong>" : NULL)."
								</td>
							</tr>
						</table>";
					while (list($key) = each($match_task)) {
						...
						<table cellspacing=\"0\" cellpadding=\"5\">";
							...
							for ($j = 0; $j < count($otherAppts); $j++) {
								...
								<tr>
									<td>
										<div >
										<font color=\"".$this->setColor($this->current_lot['sched_status'][$appt_keys[$j]],$this->current_lot['task'][$appt_keys[$j]])."\">
											".$row['title']."&nbsp;$time
										</font>
										</div>
									</td>
	...
	*/////////////////////////////////////////////////////////////////////////////////////
	function WriteHead($StartDate,$colspan,$DaysToAd,$community=NULL,$lotHashIn=NULL) {
		...
		<table cellspacing=\"1\" class=\"sched_inside\" >
			<tr>
				<td colspan=$colspan class=\"sched_head\">
	...
	*/////////////////////////////////////////////////////////////////////////////////////
	function undoDisplay() {
		...
		$result = $db->query("SELECT COUNT(*) As Total 
							  FROM `lots` 
							  WHERE `id_hash` = '".$this->current_hash."' && `lot_hash` = '".$this->lot_hash."' && `undo_task` != ''");
	...
	*/////////////////////////////////////////////////////////////////////////////////////
	function moveTasks($NewMasterTask,$NewMasterPhase,$origTask,$origPhase,$origDuration,$moveDaysArray,$durationStart,$statusArray,$durationChange=NULL,$origElement=NULL) {
		...
		for ($i = $durationStart; $i < count($NewMasterTask); $i++) {			
			...
			//echo $i." : ".$this->getTaskName($NewMasterTask[$i])." (".$NewMasterTask[$i].")...";
			...
			if (!$durationChange || $durationChange > 0) {
				...
				//echo "OrigPhase: ".$origPhase[$i]."...LastPhase: ".$lastPhase."...";
				if ($x <= 1) {
					//echo "x <= 1...";
			...
			//echo "Updated Phase: ".$NewMasterPhase[$i]."<br />";
		}
