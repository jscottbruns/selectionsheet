...
class appt {
	...
	function addAppt() {
		...
		switch ($option1)
		{
			// DAYS
			case "1":
			$calendar_array = $this->day_option($option0, $sel_date);
			break;
		
			// WEEKS
			case "2":
			$calendar_array = $this->week_option($option0, $sel_date);
			break;
		
			// MONTHS
			case "3":
			$calendar_array = $this->month_option($option0, $sel_date);
			break;
		
			// YEARS
			case "4":
			$calendar_array = $this->year_option($option0, $sel_date);
			break;
		
			// SELECTED DAYS (Mon, Wed, Fri)
			case "5":
			$freqarr = array('0', '2', '4');
			$calendar_array = $this->every_selected_day($option0, $freqarr, $week_num - 1);
			break;
	...
	function month_option($option0, $sel_date)
	{
		...
		while( $disptime < $endtime )
		{
			...
			if($option0 == 3)
			{
				...
				if($month_ptr > 12)
				{
					$month_ptr = $this->get_month($month_ptr);
					$cal_year++;
				}
			}
			if($option0 == 4)
	...
	function updateWithOutRepeatChange($title,$event,$unixDay,$unixDate,$location,$community,$lot_no,$notes,$reminderTimes,$reminderDest,$repeatStr,$endDate,$apply,$updateID) {
		...
		if ($apply == "future" && is_array($old_updateArray)) {
			$this->updateRepeatID($old_updateArray,$newEndDate);
		}
		
		if ($apply == "all" && is_array($updateArray)) {
			$this->updateRepeatID($updateArray);
		}
		if (is_array($deleteArray)) {
			$this->deleteAppointments($deleteArray);
		}
	...
	function deleteRepeatingEvents($updateID,$apply) {
		...
		if ($apply != "all") {
			$repeat_id = array_values($repeat_id);
			$this->updateRepeatID($repeat_id,$newEndDate);
		}
							...
							<table style=\"width:100%\" class=\"smallfont\">
								<tr >
									<td style=\"text-align:left;font-weight:bold;\">
										<a href=\"?start=".date("Y-m-d",strtotime("$CurrentDate $DaysToAd[$i]"))."&view=day\" style=\"color:#0A58AA;text-decoration:none;\">".date("d",strtotime ("$CurrentDate $DaysToAd[$i]"))."</a>
									</td>
									<td style=\"text-align:right\">
										[<a style=\"color:#0A58AA;text-decoration:none;\" href=\"?cmd=add&cal_month=".date("m",strtotime ("$CurrentDate $DaysToAd[$i]"))."&cal_day=".date("d",strtotime ("$CurrentDate $DaysToAd[$i]"))."&cal_year=".date("Y",strtotime ("$CurrentDate $DaysToAd[$i]"))."&start=".$_REQUEST['start']."&view=".$_REQUEST['view']."\"><small>add</small></a>]
									</td>
								</tr>
								<tr>
	...
	function dailyApptCal($SchedDate) {
		...
		list($early,$late) = $this->getDailyStartHour($SchedDate);
					...
					<tr class=\"msgnew\">
						<td style=\"border-width: 0 0 1px 0;border-style: solid;padding:2px 0 2px 5px;vertical-align:center;\">
							<table class=\"smallfont\" style=\"width:100%;\">
								<tr>
									<td style=\"width:40px;text-align:right;font-weight:bold;color:#0A58AA;\"></td>
									<td >
										<table class=\"smallfont\" cellpadding=\"5\" style=\"width:50%;background-color:#E1EBFB;border-width:0 0 1 1;border-style:solid;border-color:black;\">
											<tr>
												<td><strong>".$time[$i]. $reminder[$i]."<br/><a href=\"?cmd=add&eventID=".base64_encode($obj_id[$i])."\">".$title[$i]."</a></strong><br>".$title[$j]."</td>
											</tr>
										</table>
									</td>
								</tr>
							...
							<table class=\"smallfont\" style=\"width:100%;\">
								<tr>
									...
									<td >
										<table class=\"smallfont\" cellpadding=\"5\" style=\"width:50%;background-color:#E1EBFB;border-width:0 0 1 1;border-style:solid;border-color:black;\">
											<tr>
												<td onClick=\"window.location='?cmd=add&eventID=".base64_encode($obj_id[$i])."'\" style=\"cursor:pointer;\"><strong>".$time[$i].$reminder[$i]."<br />".$title[$i]."</strong><br>".$title[$j]."</td>
											</tr>
										</table>
									</td>
								</tr>
		...
		for ($i = $early; $i <= $late; $i++) {
			...
			<tr class=\"msgnew\">
				<td style=\"border-width: 0 0 1px 0;border-style: solid;padding:2px 0 2px 5px;vertical-align:center;\">
					<table class=\"smallfont\" style=\"width:100%;\">
						<tr>
							<td style=\"width:40px;text-align:right;font-weight:bold;color:#0A58AA;\">".date("ga",$exactTime).":</td>
							<td >";
