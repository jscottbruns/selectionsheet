...
class chart {
	...
	var $chart_data;
	var $chart_root;
	var $public_chart_root;
	var $swf_file;
	var $chart_lib;
	var $chart_license = "G1XIRHPXKE7L.NS5T4Q79KLYCK07EK";
	function chart($hash=NULL) {
		if ($hash)
			$this->hash = $hash;
		else 
			$this->hash = $_SESSION['id_hash'];
			
		$this->chart_root = SITE_ROOT."core/charts/chart_data/";
		$this->public_chart_root = LINK_ROOT."core/charts/chart_data/";
		$this->swf_file = LINK_ROOT."core/charts/charts.swf";
		$this->chart_lib = LINK_ROOT."core/charts/charts_library";
	}
	
	function load_chart($chart) {
		$this->chart_data = $chart;
		
		$file_name = "chart_data_".$this->hash.".php";
		while (file_exists($this->chart_root.$file_name))
			$file_name = "chart_data_".$this->hash.rand(500,500000).".php";
			
		$this->SendChartData($file_name);
		
		return $this->public_chart_root.$file_name;
	}

	function InsertChart($php_source, $width=400, $height=250, $bg_color="666666", $transparent=false) {
		$php_source=urlencode($php_source);
		$library_path=urlencode($library_path);
		$flash_file = $this->swf_file;
		$library_path = $this->chart_lib;
		$license = $this->chart_license;
		
		$rand = rand(1000,5000000);
		...
		$html.="<PARAM NAME=movie VALUE='".$flash_file.$u."library_path=".$library_path."&php_source=".$php_source."&r=".$rand;
		...
		$html.="<EMBED src='".$flash_file.$u."library_path=".$library_path."&php_source=".$php_source.".&r=".$rand;
		if($license!=null){$html.="&license=".$license;}
	...
	//====================================
	function SendChartData ($file_name) {
		...
		$fh = fopen($this->chart_root.$file_name,"w+");
		fwrite($fh,"<?php
