...
if (!defined('PUN'))
	die('The config file is corrupt');;
//
...
class DBLayer
{
	...
	function query($sql, $unbuffered = false)
	{
		...
		if ($unbuffered)
			$this->query_result = @mysql_unbuffered_query($sql, $this->link_id)or (write_error(debug_backtrace(),"Error in query:\n ".$sql."\n\n".mysql_error()));
		else
			$this->query_result = @mysql_query($sql, $this->link_id)or (write_error(debug_backtrace(),"Error in query:\n ".$sql."\n\n".mysql_error()));
